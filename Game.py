...
class Ship(pygame.sprite.Sprite):
        ...
        def hyperspace(self):
            ship.center = (random.randint(0,size[0]), random.randint(0,size[1]))
        ...
        def collision_bullet(self, bullet, scoreboard):
            offset_bullet = (int(self.center[0] - bullet.start_point[0]), int(self.center[1] - bullet.start_point[1]))
            overlap_bullet = self.mask.overlap(bullet.mask, offset_bullet)
            return ship_rebirth(overlap_bullet, self, asteroid_list, scoreboard)
...
class Bullet(pygame.sprite.Sprite):
    def __init__(self, surface, angle, position, color, length):
        ...
        self.x_trig_coeff = round(self.velocity * math.cos(math.pi * (self.angle/180)),5)
        self.y_trigg_coeff = self.velocity * math.sin(math.pi * (self.angle/180))
        self.BULLET = pygame.Surface((40, 40), pygame.SRCALPHA)
        pygame.draw.line(self.BULLET, self.color, (20, 20), self.end_point,1)
    ...
    def move(self):
        self.start_point = (self.start_point[0] + self.x_trig_coeff, self.start_point[1] - self.y_trigg_coeff)
        self.end_point = (self.x_trig_coeff, -self.y_trigg_coeff)
...
class Asteroid(pygame.sprite.Sprite):
    ...
    def collision_ship(self, ship, scoreboard):
        ...
        return ship_rebirth(overlap_asteroid, ship, asteroid_list, scoreboard)
...
class UFO(pygame.sprite.Sprite):
    ...
    def collision_ship(self, ship, scoreboard):
        ...
        return ship_rebirth(overlap_ship, ship, asteroid_list, scoreboard)

    def collision_bullet(self, bullet):
        offset_asteroid = (int(bullet.start_point[0] - self.position[0] - 15), int(bullet.start_point[1] - self.position[1] - 15))
        overlap_asteroid = self.mask.overlap(bullet.mask, offset_asteroid)
...
def ship_rebirth(overlap, ship, asteroid_list,scoreboard):
...
def main():
    ...
    while not close_clicked:
        ...
        for event in pygame.event.get():
            ...
            if main_menu_display and main_menu.get_rect()[0] <= pygame.mouse.get_pos()[0] <= main_menu.get_rect()[1] and main_menu.get_rect()[2] <= pygame.mouse.get_pos()[1] <= main_menu.get_rect()[3] and event.type == pygame.MOUSEBUTTONUP:
        ...
        if continue_game and main_menu_display == False:
            ...
            for i in ufo_bullet_list:
                a = ship.collision_bullet(i, scoreboard)
                if a:
                    continue_game = not a
                for j in asteroid_list:
                    if j.collision_bullet(i) != None:
            ...
            if ufo != None:
                bool = ufo.collision_ship(ship, scoreboard)
                if bool:
                    continue_game = False
                else:
                    continue_game = True
            ...
            for i in asteroid_list:
                c = i.collision_ship(ship, scoreboard)
                if c:
                    continue_game = not c
